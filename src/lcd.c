...
void lcd_make(lcd_t * lcd, uint8_t addr, uint8_t cols, uint8_t rows, uint8_t sda_pin, uint8_t scl_pin) {
  ...
  lcd->_i2c = i2c0; // i2c_default; // &i2c0_inst;
...
// PICO_DEFAULT_I2C_SDA_PIN
  ...
  // metadata: make I2C pins avaiable to picotools & pico_info
  // I2C: GPIO ? (SDA), GPIO ? (SCL)
...
void lcd_init(lcd_t * lcd) {
  ...
  // 8bit to 4bit mode
  lcd_write_command(lcd, 0x03); // first
  lcd_write_command(lcd, 0x03); // repeat
  lcd_write_command(lcd, 0x03); // final
  // confirm 4bit mode
